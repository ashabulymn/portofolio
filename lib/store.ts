...
export const contentSchema = z.object({
  ...
  entries: z.array(z.object({ id: z.string().regex(/^[a-zA-Z0-9_-]{1,80}$/), category: z.string().max(50), title: text, subtitle: text, body: text, order: z.number().int().min(0), visible: z.boolean(), featured: z.boolean() })).max(500),
});
