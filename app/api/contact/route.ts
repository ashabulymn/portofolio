...
export async function POST(request: Request) {
  ...
  try {
    ...
    const db = database();
    try { db.prepare('INSERT INTO inquiries (name,email,category,message,created) VALUES (?,?,?,?,?)').run(data.name, data.email, data.category, data.message, new Date().toISOString()); }
    finally { db.close(); }
