...
import { currentAdmin, readJson, sameOrigin } from '@/lib/security';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!length || length > 6 * 1024 * 1024) return new Response('File too large', { status: 413 });
    ...
    const kind = pdf ? 'cv' : 'image'; const mime = pdf ? 'application/pdf' : png ? 'image/png' : jpg ? 'image/jpeg' : 'image/webp';
    const language = form.get('language') === 'en' ? 'en' : 'id';
    const id = randomUUID(); await mkdir(directory(), { recursive: true }); await writeFile(path.join(directory(), id), bytes, { flag: 'wx' });
    const db = database();
    try { db.prepare('INSERT INTO media (id,filename,mime,title,alt,language,kind,created) VALUES (?,?,?,?,?,?,?,?)').run(id, file.name.slice(0,150), mime, String(form.get('title') || file.name).slice(0,200), String(form.get('alt') || '').slice(0,500), language, kind, new Date().toISOString()); }
